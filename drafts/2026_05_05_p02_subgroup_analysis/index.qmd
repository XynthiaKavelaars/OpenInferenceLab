...
csl: apa.csl
categories: [Bayesian analysis, multivariate analysis, decision rules, subgroup analysis, bmco]
...
*This vignette is part of the `bmco()` package and can be found [here](https://cran.r-project.org/web/packages/bmco/vignettes/subgroup-analysis.html)*
...
For Bayesian logistic regression priors: - Gelman, A. Jakulin, A., Pittau, M. G. & Su, Y-S (2008). A weakly informative default prior distribution for logistic and other regression models. *Annals of Applied Statistics, 2*(4), 1360-1383. \doi{10.1214/08-AOAS191}.

## MCMC Diagnostics

The function internally checks the multivariate potential scale reduction factor (MPSRF) and warns if \> 1.1. Additional MCMC diagnostics will be returned When `return_diagnostics = TRUE`. When `return_diagnostic_plots = TRUE`, `bglm()` returns diagnostic plots as well.
...
`r format_apa(bib[["Kavelaars2024"]])`
